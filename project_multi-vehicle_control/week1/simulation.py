...
def sim_run(options, simulation_options, MPC, save=False, name=None, show=True):
    ...
    for i in range(1, sim_total+1):
        ...
        for j in range(1, mpc.horizon):
            predicted = mpc.plant_model(
                predicted_state[-1], mpc.dt, u[2*j], u[2*j+1])
    ...
    def update_plot(num):
        patch_car.set_xy(car_patch_pos(
            state_i[num, 0], state_i[num, 1], state_i[num, 2]))
    ...
    def plot_trajectory(points, save=False, name=None):
        ref_1 = mpc.reference1
        ...
        cbar.ax.set_ylabel("Throttle", fontsize=15)
        ...
        # costs as information on the plot
        y_value = 13
        for key, value in simulation_options.items():
            if "cost" in key:
                plt.text(-2, y_value, key + ": " + str(value), fontsize=12)
                y_value -= 0.5

        # plot start and goal
        patch_car_traj = mpatches.Rectangle(
            (0, 0), car_width, 2.5, fc='k', fill=False)
        patch_car_traj.angle = np.rad2deg(0)-90
        patch_goal_traj = mpatches.Rectangle(
            (ref_1[0], ref_1[1]), car_width, 2.5, fc='k', ls='dashdot', fill=False)
        patch_goal_traj.angle = np.rad2deg(ref_1[2])-90
    ...
    # Animation.
    file_name = "-".join([key + "_" + str(value)
                         for (key, value) in simulation_options.items()])
