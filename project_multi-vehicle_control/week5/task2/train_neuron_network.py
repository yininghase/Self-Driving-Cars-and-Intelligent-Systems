...
if __name__ == "__main__":
    ...
    model = NeuronNetwork(input_size=7*NUM_VEHICLES, output_size=2*HORIZON*NUM_VEHICLES).to(device)
    # X, _ = load_data("./data/")
    # X = generate_data(DATA_LENGTH, NUM_VEHICLES, position_range=10)
    X = generate_data_with_collision(DATA_LENGTH, NUM_VEHICLES, position_range=10)
    X.reshape(-1, NUM_VEHICLES, 7)
    ...
    # Where these the values that we were using earlier?
    distance_cost=1
    angle_cost=1
    velocity_cost=0
    smoothness_cost=0.1
    traveled_distance_cost=0.1
    collision_cost = 1

    criterion = Loss(HORIZON, NUM_VEHICLES, distance_cost, angle_cost, velocity_cost, 
                     smoothness_cost, traveled_distance_cost, collision_cost)
