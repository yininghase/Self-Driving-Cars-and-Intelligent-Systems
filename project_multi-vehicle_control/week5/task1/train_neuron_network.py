...
# from nn import SkipConnectionNetwork
from generate_data import get_combined_data_b


out_folder = './week5/task1/models/relearning/'
...
def train():
    ...
    # model = SkipConnectionNetwork(input_size=14, output_size=2*20*2, hidden_sizes=[30, 200, 100]).to(device)
    model = NeuronNetwork(input_size=7, output_size=2*20*1, hidden_sizes=[30, 200, 100]).to(device)
    # X, y = load_data("./week5/task1/data/") 
    X, y = get_combined_data_b()
    print(X.shape)
    print(y.shape)
    ...
    # scheduler_1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=4, eta_min=0)
    # scheduler_2 = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda epoch: 0.97**epoch)
    lrs = []

    model_name = "test"

    criterion = WeightedMeanSquaredError(batch_size, horizon, num_vehicles)
    # exponential weights as this was the best perfoming model
    ...
    plt.legend()
    plt.savefig(os.path.join(out_folder, model_name + "_learning_plot_best_val_{}.png".format(best_loss)))
    plt.show()
    plt.plot(lrs)
    plt.savefig(os.path.join(out_folder, model_name + "_learning_rate_plot.png"))
    plt.show()
