...
class Visualization:
    def __init__(self, num_vehicles, is_model, simulation_length, horizon, start, target, name, show, save, show_optimization):
        self.figure_size = 8
        ...
        self.folder = "./week5/task1/plots/model1"
        # self.folder = "./plots/model1"
    ...
    def create_video(self, data, predict_opt, predict_model):
        ...
        if self.is_model:
            self.predict_model = np.concatenate(predict_model[1:], axis=0).reshape(-1, self.horizon, self.num_vehicles, 4)
        if self.show_optimization:
            if torch.is_tensor(predict_opt):
                self.predict_opt = predict_opt[:, :, :, :2]
            else:
                self.predict_opt = np.concatenate(predict_opt[1:], axis=0).reshape(-1, self.horizon, self.num_vehicles, 4)
